...
void runSkillsAuto(){
  ...
  goOldPID(21.5, 1, 0.09, 80, 0.05);
  upMove(BACKWARD, FAST, 170);
  intakeMove(BACKWARD, FAST, 370);
  upMove(BACKWARD, 90, 820);
  goSides(FORWARD, FORWARD, 30, 30, 4.2, 4.2);
  intakeMove(FORWARD, FAST, 370);
  intakeSpin(FORWARD, 100);
  upMove(FORWARD, MEDIUM, 450);
  intakeSpin(FORWARD, 70);
  wait(200);
  //cube 7
  turnEncoder(LEFT, 20, 141);
  goo(FORWARD, 25, 47, 29, 0);
  goH(LEFTH, 100, 6.5);
  while(RF.isSpinning()){
    wait(1);
  }
  ...
  intakeMove(FORWARD, FAST, 200);
  //tower 1
  goH(RIGHTH, 100, 6.5);
  goSides(FORWARD, FORWARD, 26, 26, 3.2, 3.2);
  upMove(BACKWARD, 75, 400);
  intakeMove(BACKWARD, FAST, 200);
  goOldPID(50, 1,  0.15, 40, 0.25);
  ...
  //red
    if (autoColor == 0){
      ...
      if (autoNumber == 1){
        ...
        //back
        resetDrive();
        goEncoder(BACKWARD, 60, 16.5);
        goH(LEFTH, 80, 2);
        resetDrive();
        /*while(RF.isSpinning()){
          wait(1);
        }*/
        goSides(BACKWARD, BACKWARD, 80, 40, 38, 7);
        /*while((LF.isSpinning())||(RF.isSpinning())){
          wait(1);
        }*/
        //resetDrive();
        //turnEncoder(LEFT, 40, 117);
        //place cubes
        goo(FORWARD, 60, 35, 0, 0);
        while(RF.isSpinning()){
          wait(1);
        }
        wait(400);
        goo(FORWARD, 60, 2, 0, 0);
        while(RF.isSpinning()){
          wait(1);
        }
        //upMove(BACKWARD, 70, 130);
        intakeMove(BACKWARD, FAST, 650);
        upMove(FORWARD, 30, 170);
        goo(BACKWARD, 60, 3, 0, 0);
        while(RF.isSpinning()){
          wait(1);
        }
        stopEverything();
