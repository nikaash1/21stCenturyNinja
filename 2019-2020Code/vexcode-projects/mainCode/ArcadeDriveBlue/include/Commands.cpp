  ...
  //red
    if (autoColor == 0){
      ...
      if (autoNumber == 3){
        ...
        upMove(BACKWARD, 70, 330);
        //tower1
        goH(RIGHTH, 400, 5);
        upMove(BACKWARD, 75, 70);
        goOldPID(46, 1.0,  0.15, 40, 0.25);
    ...
    if (autoColor == 1){
      if (autoNumber == 1){
        ...
        wait(200);
        //cube 7
        turnEncoder(RIGHT, 32, 131);
        goo(FORWARD, 54, 46, 35, 0);
        goH(RIGHTH, 100, 8);
        while(LF.isSpinning()){
          wait(1);
        }
